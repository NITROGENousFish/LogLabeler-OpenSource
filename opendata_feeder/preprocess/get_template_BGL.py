...
def bgl_preprorcess(rawlog:str):
    rawlog = rawlog.strip()
    res_is_anomaly = True if not rawlog.startswith("-") else False
    res_log = rawlog[2:] if not res_is_anomaly else rawlog
    actual_log = res_log.split(" ",8)[-1]
    return res_is_anomaly, actual_log

def main_miner(bgl_log_data,drain_config_path):
    # * drain3 init
    ...
    for rawlog in tqdm(raw_loglist):
        is_anomaly,log_line = bgl_preprorcess(rawlog)       #* preprocess BGL
...
if __name__ == "__main__":
    bgl_log_data = PROJ_ABS_FILEPATH+"./opendata_feeder/original_datasets/BGL/BGL.log"
    drain_config_path = PROJ_ABS_FILEPATH+"./opendata_feeder/data/drain_configs/drain_BGL_simth08.ini"
    BGL_WORK_FOLDER = PROJ_ABS_FILEPATH+"./opendata_feeder/data/output_BGL"
    os.makedirs(BGL_WORK_FOLDER,exist_ok=True)
    miner,res_template_id_hash,_drain_cluster_id_to_is_anomaly = main_miner(bgl_log_data,drain_config_path)
