...
VOCAB_PATH = os.path.join(_cur_dir,"glove.6B.300d.txt")
...
class Simple_template_TF_IDF(EmbeddingInterface):
    total_words = 0
    num_oov = 0
    ...
    def embedding_one(self, logstr: str) -> np.ndarray:
        print("cannot embedding one logstr since tfidf need all vocab, please use embedding_list")
        raise NotImplementedError

    def presents(self, id2templates):
        """
        ...
        return id2embed
    def embedding_list(self,template: List[str]) -> List[np.ndarray]:
        #remove <~~> pairs in template
        template = list(map(lambda x: re.sub(r'<~.*?~>', '', x), template))
        res = self.presents({k: v for k, v in enumerate(template)})
...
def test_Simple_template_TF_IDF():
    ...
    resid2embed = tfidf.embedding_list(loglist)
    assert isinstance(resid2embed, list)
