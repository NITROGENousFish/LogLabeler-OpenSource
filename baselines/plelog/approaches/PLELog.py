...
class PLELog:
    ...
    def evaluate(self, instances, threshold=0.5):
        """
        instances: list of plelog.entities.instances.Instance object
        list(filter(lambda x:x[1].label=='Anomalous',[(idx,i) for idx, i in enumerate(instances)]))
        """
        ...
        with torch.no_grad():
            ...
            for onebatch in data_iter(instances, self.test_batch_size, False):
                anomalous_num_this_batch = len(list(filter(lambda x:x[1].label=='Anomalous',[(idx,i) for idx, i in enumerate(onebatch)])))
                if anomalous_num_this_batch != 0:
                    print(anomalous_num_this_batch)
                tinst: TInstWithLogits = generate_tinsts_binary_label(
                    onebatch, self.vocab
                )
                ...
                for inst, bmatch in batch_variable_inst(
                    onebatch, pred_tags, tag_logits, {0: "Normal", 1: "Anomalous"}
                ):
                    tag_total += 1
                    if bmatch:
                        tag_correct += 1
            ...
            if TP == 0:
                self.logger.info('TP is 0, No anomaly detected, precision and recall are both 0')
...
if __name__ == '__main__':
    ...
    # * place HDFS and BGl under datasets/[NAME] dir
    ...
    # * zongyang: Simple_template_TF_IDF
    ...
    if reduce_dimension != -1:
        ...
        if addedtool.is_have_cache("fastica.pkl"):
            train_reprs = addedtool.cache_load("fastica.pkl")
        else:
            train_reprs = _transformer.fit_transform(train_reprs)
            addedtool.cache_save(train_reprs,"fastica.pkl")
    ...
    labeled_train = None
    if addedtool.is_have_cache("labeled_train.pkl"):
        labeled_train = addedtool.cache_load("labeled_train.pkl")
    else:
        labeled_train = label_generator.auto_label(train, normal_ids)
        addedtool.cache_save(labeled_train, "labeled_train.pkl")
