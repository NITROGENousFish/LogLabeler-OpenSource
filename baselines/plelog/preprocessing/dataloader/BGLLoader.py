...
inconsis_num=0
# * Add to justify
def is_homogeneous_bool(lst):
    if not lst:  # 如果列表为空，认为它是“同质的”
        return True
    first_elem = lst[0]
    return all(elem == first_elem for elem in lst)
# * Add to justify
def count_homogenous(lst,gth):
    global all_num
    global inconsis_num
    if lst:
        all_num += len(lst)
        inconsis_num += [i!=gth for i in lst].count(True)
...
class BGLLoader(BasicDataLoader):
    ...
    def _load_raw_log_seqs(self):
        """
        #* raw_log_seqs.txt format [block_id]:[seqlist(splited by space)]
            self.blocks -> list[block_id]
            self.block2seqs -> dict{block_id:list[line_id]}
        # * label.txt format self.block2label:str(label in either 'Anomaly','Normal')
            self.block2label -> dict{block_id:str(label in either 'Anomaly','Normal')}

        """
        # * sequence_file -> self.block2seqs, self.blocks
        sequence_file = os.path.join(self.dataset_base, 'raw_log_seqs.txt')
        # * label_file -> self.block2label
        ...
        if os.path.exists(sequence_file) and os.path.exists(label_file):
            ...
            with open(self.in_file, 'r', encoding='utf-8') as reader:
                ...
                nodes = OrderedDict() #* node for actual machine node like R02-M1-N0-C:J12-U11
                #* session_window in PLElog
                ...
                for node, seq in nodes.items():
                    if len(seq) < self.window_size: #todo why windows size set 120?
                        ...
                        for (idx, line) in seq:
                            ...
                            if not line.startswith('-'):
                                label = 'Anomalous'
                        # assert is_homogeneous_bool([line.startswith('-') for (_,line) in seq])==True
                        count_homogenous([line.startswith('-') for (_,line) in seq],label=='Normal')
                        ...
                        while i < len(seq):
                            ...
                            for (idx, line) in seq[i:i + self.window_size]:
                                ...
                                if not line.startswith('-'):
                                    label = 'Anomalous'
                            # assert is_homogeneous_bool([line.startswith('-') for (_,line) in seq[i:i + self.window_size]])==True
                            count_homogenous([line.startswith('-') for (_,line) in seq[i:i + self.window_size]],label=='Normal')
                ...
                pbar.close()
                print(f"JUSTIFY-WINDOW12-SETTING:all_num:{all_num},inconsisnum:{inconsis_num},wrong ratio:{inconsis_num/all_num}")
