...
def generate_tinsts_binary_label(batch_insts, vocab, if_evaluate=False):
    ...
    b = 0
    # [i.predicted for i in batch_insts[:20]]
...
def generate_tinsts_binary_label_modified(onebatch, vocab):
    slen = len(onebatch[0].sequence)
    batch_size = len(onebatch)
    for b in range(1, batch_size):
        cur_slen = len(onebatch[b].sequence)
        if cur_slen > slen:
            slen = cur_slen
    tinst = TInstWithLogits(batch_size, slen, 2)
    b = 0
    # [i.predicted for i in onebatch[:20]]
