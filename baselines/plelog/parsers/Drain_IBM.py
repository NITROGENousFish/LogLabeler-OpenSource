...
class Drain3Parser():
    ...
    def parse_file(self, in_file, remove_cols=None, clean=False): #* -> Iterable(drain.LogCluster)
        ...
        #* dict_values of drain.LogCluster obj, same as: list(drain.LogCluster)
        #* 外部usage: list(ret_val)
        return self.parser.drain.clusters

    def remove_columns(self, line, remove_cols, clean=False):
        #* 按照空格切分, 删除remove_cols中idx的列; 如果clean, 删除\*.?+$^[](){}|/这些原始字符
        tokens = line.split()
        after_remove = []
    ...
    def parse_line(self, in_line, remove_cols=None, save_right_after=False): #* -> Iterable(drain.LogCluster)
        line = in_line.strip()
        if remove_cols:
    ...
    def match(self, inline): #* -> drain.LogCluster
        #* ret type:
