...
def session_window(data_dir, datatype, sample_ratio=1):
    ...
    for i in tqdm(range(len(train_df))):
        ...
        Sequential_pattern = _trp(ori_seq, 50)
        Semantic_pattern = []
        for event in Sequential_pattern:
            if event == 0:
                Semantic_pattern.append([-1] * 300)
...
def modify_custom_embedding_with_fixed_window(data_dir, id2template_path):
    from logdeep.tools.embedding import fast_tfidf_embedding
    id2template = pd.read_csv(id2template_path)
    def eventid_remove_E_and_to_int(s:str)->int:
        assert s[0] == "E"
        return int(s[1:])
    eventids: list = [
        eventid_remove_E_and_to_int(i) for i in list(id2template["EventId"])
    ]

    eventtemplates:list = list(id2template["EventTemplate"])
    event2semantic_vec = {
        i: emb for i, emb in zip(eventids, fast_tfidf_embedding(eventtemplates))
    }

    result_logs = {}
    result_logs["Sequentials"] = []
    # result_logs["Quantitatives"] = [] #* manual disable Quantitatives cuz thunderbird is too sparse
    result_logs["Semantics"] = []
    labels = []
    ...
    for i in tqdm(range(len(train_df))):
        ori_seq = [
            eventid_remove_E_and_to_int(eventid)
            for eventid in train_df["sequence"][i].split(" ")
        ]
        Sequential_pattern = _trp(ori_seq, 100) #* zongyang: padding to 100
        Semantic_pattern = []
        for event in Sequential_pattern:
            if event == 0:
                Semantic_pattern.append([-1] * 300)
        ...
        # * manual disable Quantitatives cuz thunderbird is too sparse
        # Quantitative_pattern = [0] * len(event2semantic_vec)
        # log_counter = Counter(Sequential_pattern)
        ...
        Sequential_pattern = np.array(Sequential_pattern)[:, np.newaxis]
        # Quantitative_pattern = np.array(Quantitative_pattern)[:, np.newaxis] #* manual disable Quantitatives cuz thunderbird is too sparse
        result_logs["Sequentials"].append(Sequential_pattern)
        # result_logs["Quantitatives"].append(Quantitative_pattern) #* manual disable Quantitatives cuz thunderbird is too sparse
        result_logs["Semantics"].append(Semantic_pattern)
        labels.append(int(train_df["label"][i]))
...
if __name__ == "__main__":
    a,b = modify_custom_embedding_with_fixed_window(
        "data/sampling_example/bgl/BGL_sequence.csv",
        "data/sampling_example/bgl/templates.csv"
    )
    breakpoint()
