...
from logdeep.dataset.sample import sliding_window, session_window, modify_custom_embedding_with_fixed_window
...
class Trainer():
    def __init__(self, model, options):
        ...
        train_logs, train_labels = modify_custom_embedding_with_fixed_window(
        ...
        self.num_valid_log = len(train_dataset)  #! should be valid_dataset

        print(f"Find {self.num_train_log} train logs")
        print(f"Find {self.num_valid_log} train logs")
    ...
    def train(self, epoch):
        ...
        for i, (log, label) in enumerate(self.train_loader):
            features = []
            # * zongyang: 这里拼接了seqentials,quantitatives,semantics三个
    ...
    def start_train(self):
        print(f"{__file__}: start training, start epoch:{self.start_epoch}, max epoch:{self.max_epoch}")
